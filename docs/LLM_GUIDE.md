...
| Wan-S2V (audio+img → video) | wan_2.1_vae, umt5-xxl, wav2vec2 audio encoder | 6.0 | 30 | euler | discrete (flow_shift=3) | 832×480, 33 frames @ 16fps |
| MiniT2I | flan-t5-large | 6.0 | 100 | euler | discrete | 512×512 |
| SenseNova-U1.5 | — (self-contained HF repo dir; no separate VAE / text encoder) | 4.0 | 50 | euler (`cuda` RNG, `flow_shift=3`) | discrete | 2048×2048 (multiples of 32) |
| Qwen-Image-2.1 | qwen_image_2.1_vae_bf16, Qwen3-VL-8B-Instruct (mmproj for edit) | 6.0 | 20 | euler | discrete | 1024×1024 |
| LLaDA-Image (base) | llada_vae, LLaDA2-MoE text encoder, embeddings_connectors | 4.0 | 50 | euler | **llada_image** | 1024×1024 |
| LLaDA-Image-Turbo | same components as base (different transformer + encoder + connectors) | 4.0 | 4 | euler | llada_image | 1024×1024 |

**Full list of architecture keys:** Anima, AnimateDiff-v2, AnimateDiff-v3, Boogu-Image, Chroma, Chroma-Radiance, Ernie-Image, Flux, Flux Kontext, Flux Schnell, Flux2 Dev, Flux2 Klein 4B, Flux2 Klein 9B, Flux2 Klein Base 4B, Flux2 Klein Base 9B, HiDream-O1, HunyuanVideo-1.5, Ideogram-4, Krea2-Raw, Krea2-Turbo, Krea2OstrisEdit, LCM, LCM-SDXL, LLaDA-Image, Lens, Lingbot, Longcat-Image, LTX-2.5, LTXAV, Mage-Flow, Minimax-H3, MiniT2I, Ovis, PiD, PiD-1.5, Qwen, Qwen Image Edit, Qwen-Image-2.1, SD1, SD2, SD3, SDXL, SDXS-512, SeFi-Image-Base, SeFi-Image-RL, SeFi-Image-Turbo, SenseNova-U1.5, SSD-1B, Vega, Wan, Wan-S2V, Z-Image.

**Recent additions:** AnimateDiff-v2/v3 and PiD-1.5 use an SD 1.5 base + a `motion_module`; loaded together they enable video generation. HunyuanVideo-1.5, Lingbot, and Wan-family are video-first. Krea2OstrisEdit is a Flux2-family edit variant using `ref_image_args` for reference-image processing. Mage-Flow is a Flux-family flow model. Minimax-H3 is an image model that ships with a trained VAE latent2rgb projection so previews look decent by default. **Wan-S2V** (leejet/stable-diffusion.cpp#1925) extends the Wan 2.2 family with audio+image-to-video, adding a wav2vec2 audio encoder alongside the usual video VAE and umt5-xxl. **MiniT2I** (upstream #1683) is a small 512×512 diffusion transformer paired with flan-t5-large - useful for smoke tests and low-VRAM builds. **SenseNova U1.5** (upstream #1935) is an 8B MoT model that diffuses directly in RGB pixel space; it ships as a complete Hugging Face repository directory (index + shards + tokenizer) and needs no separate VAE or text encoder. **Qwen-Image 2.1** (upstream #1994) drops in as a Qwen-Image successor with its own qwen_image_2.1 VAE (not interchangeable with the earlier Qwen / Wan 2.2 VAEs) and Qwen3-VL-8B-Instruct; the mmproj sidecar is required to edit with a GGUF LLM. **LLaDA-Image** (upstream #1968) is a NextDiT-style 6B model conditioned by a LLaDA2-MoE diffusion-LLM, reusing the Flux.2 VAE - two checkpoints ship (base 50-step and Turbo 4-step); each has its own transformer + text encoder + connectors, only VAE and tokenizer are shared. Uses the new `llada_image` scheduler and needs an `embeddings_connectors` file (pick the `-edit` variant, which carries SigVQ, for image editing). Also new upstream: `sage_attn` load option (`--sage-attn`) turns on native CUDA SageAttention for the diffusion path on SM80+ / CUDA 12.0+.
...
**Sampler enum values:** `euler`, `euler_a`, `heun`, `dpm2`, `dpm++2s_a`, `dpm++2m`, `dpm++2mv2`, `ipndm`, `ipndm_v`, `lcm`, `ddim_trailing`, `tcd`, `res_multistep`, `res_2s`, `er_sde`, `euler_cfg_pp`, `euler_a_cfg_pp`, `euler_ge`.

**Scheduler enum values:** `discrete`, `karras`, `exponential`, `ays`, `gits`, `sgm_uniform`, `simple`, `smoothstep`, `kl_optimal`, `lcm`, `bong_tangent`, `ltx2`.
