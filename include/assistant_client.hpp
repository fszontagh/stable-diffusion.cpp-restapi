...
- load_model: Load a MAIN model with optional components in a SINGLE request.
  Parameters:
    ...
    - options: optional load options (flash_attn, enable_mmap, max_vram, disable_prefetch, disable_segmented_compute, backend, params_backend, etc.). Per-component CPU placement is via backend="diffusion=cuda0,vae=cpu"; global "all weights in RAM" is params_backend="*=cpu".
